...
function getActiveReservationsByUser(userId) {
    const now = new Date();
    return reservations.filter(r => r.userId === userId && r.endTime > now);
}
/* =======================
   POST /reservations
======================= */
app.post("/reservations", (req, res) => {
    try {
        ...
        const conflict = roomReservations.some(r => overlaps(start, end, r.startTime, r.endTime));
        if (conflict) {
...
app.get("/reservations", (req, res) => {
    ...
    if (startTime || endTime) {
        ...
        const end = endTime ? new Date(endTime) : new Date("9999-12-31");
        if (isNaN(start.getTime()) || isNaN(end.getTime())) {
            return res.status(400).json({ error: "Invalid time range" });
        }
        result = result.filter(r => overlaps(start, end, r.startTime, r.endTime));
    }
